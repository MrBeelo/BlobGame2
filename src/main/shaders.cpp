...
int Shaders::centerLoc;
int Shaders::radiusLoc;
int Shaders::intensityLoc;
...
void Shaders::LoadContent()
{
    ...
    centerLoc = GetShaderLocation(fsShaders[FX_CRT_FADED], "screenCenter");
    radiusLoc = GetShaderLocation(fsShaders[FX_CRT_FADED], "radius");
    intensityLoc = GetShaderLocation(fsShaders[FX_CRT_FADED], "intensity");
}
...
void Shaders::Update(Player *player)
{
    ...
    {
        screenCenter.x = (player->screenPos.x + player->GetSize().x / 2) / simulationSize.x;
        screenCenter.y = 1.0f - ((player->screenPos.y + player->GetSize().y / 2) / simulationSize.y);
        radius = 1.5f;
